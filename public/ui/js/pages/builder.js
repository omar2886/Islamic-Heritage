...
function linkSpouses(fam, aId, bId){
  const a = fam.people[aId];
  const b = fam.people[bId];
  if (!a || !b) return;
  ...
  if (btnChild){
    btnChild.addEventListener("click", () => {
      store.setState((s) => {
        ...
        // need sex of parent to assign fatherId/motherId deterministically
        if (cur2.sex !== "male" && cur2.sex !== "female"){
          return s;
        }

        const partial = { label: "Hijo/a", sex: "unknown", alive: true };
        if (cur2.sex === "male") partial.fatherId = sid;
        else partial.motherId = sid;

        const cid = createPerson(f, partial);
        return { ...s, builder: { ...s.builder, mode: "tree", family: f, selectedId: cid } };
      });

      const st = store.getState();
      const cur3 = st.builder?.family?.people?.[st.builder.selectedId];
      if (cur3 && cur3.sex === "unknown"){
        pushToast(store, "Define el sexo de la persona antes de añadir hijo/a (para asignar padre/madre).");
      }
    });
  }
}
